...
- Bump versions in `.github/workflows/MainDistributionPipeline.yml`
  - Set the `duckdb_version` inputs for `duckdb-stable-build` and `code-quality-check` to the latest tagged release.
  - Set the `ci_tools_version` inputs and reusable workflow tags for both jobs to the matching extension-ci-tools release branch.
